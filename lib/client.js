...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function accountRow(data) {
      ...
      // 内核的 benefit_tier 可能是字符串（"Pro"）也可能是数字枚举，两者都要认。
      var rawTier = primary ? primary.benefit_tier : undefined
      var tier
      if (typeof rawTier === "string" && rawTier.length > 0) tier = rawTier
      else if (rawTier === undefined || rawTier === null) tier = "权益未知"
      else tier = TIERS[rawTier] || "等级 " + rawTier
      return {
        tone: "ok",
    ...
    function AIStudioPage(props) {
      ...
      var importMode = react.useState("login")
      ...
      var rootOptions = [{ value: "", label: "— 请用下方路径栏指定 —" }].concat(
        browserRoots.map(function (entry) { return { value: entry.path, label: entry.name + " · " + entry.path } })
      )
      var rootInList = browserRoots.some(function (entry) { return entry.path === value.browserRoot })
      // 邮箱是账户目录名；已有账户时直接回填，避免重新登录时因为名字对不上而替换不掉旧目录。
      var firstAccountEmail = (data && Array.isArray(data.accounts) && data.accounts[0] && data.accounts[0].id)
        ? String(data.accounts[0].id)
        : ""
      var effectiveEmail = (value.accountEmail && String(value.accountEmail).trim()) || firstAccountEmail
      var canImport = importMode[0] === "login"
        ? true
        : !!(effectiveEmail && (importMode[0] === "file" ? value.storageStatePath : value.browserRoot))
      ...
      return h(
        ...
        h(
          Card,
          { title: "账户" },
          h("p", { className: "gws-hint" }, "内核用账户目录里的登录态去换 WAA proof。推荐「隔离登录」：会话由内核自己的 Camoufox 持有，与生成时的 WAA 页面同一指纹，能长期存活。「从文件导入」只导入一次性 cookie，几小时内就会失效。"),
          h(
            "div",
            { className: "gws-fields" },
            importMode[0] === "login"
              ? h("p", { className: "gws-hint" }, "点「打开登录窗口」会弹出一个独立的 Camoufox 浏览器窗口，请在其中登录 Google 并进入 AI Studio；登录完成后窗口自动关闭，账户随即写入（最长等待 10 分钟）。")
              : null,
            h(Field, {
              label: importMode[0] === "login" ? "Google 邮箱（用于替换同名账户，首次可留空）" : "Google 邮箱（账户标识）",
            ...
            importMode[0] === "browser"
              ? h(Select, {
                  label: "检测到的浏览器 User Data 目录",
                  value: rootInList ? value.browserRoot : "",
                  disabled: disabled || browserRoots.length === 0,
                  options: rootOptions,
            ...
            importMode[0] === "browser"
              ? h(Field, {
                  label: "或直接填写 User Data 目录",
                  value: value.browserRoot || "",
                  placeholder: "C:\\\\Users\\\\<你>\\\\AppData\\\\Local\\\\Microsoft\\\\Edge\\\\User Data",
                  disabled: disabled || !config.writable,
                  onChange: setField("browserRoot")
                })
              : null,
            importMode[0] === "file"
              ...
              : null,
            importMode[0] === "file"
              ? h("p", { className: "gws-hint" }, "注意：这条路径没有续签材料，Google 的 DBSC 票据作废后需要重新导出，属于临时手段。")
              : null
          ),
          h(
            "div",
            ...
            h(Button, {
              disabled: disabled,
              onClick: function () { importMode[1]("login") }
            }, importMode[0] === "login" ? "● 隔离登录（推荐）" : "○ 隔离登录（推荐）"),
            h(Button, {
              disabled: disabled,
            ...
            h(Button, {
              ...
              onClick: function () {
                run("import", function () {
                  if (importMode[0] === "login") return call("/account/import", "POST", { mode: "login", email: effectiveEmail })
                  return call("/account/import", "POST", importMode[0] === "browser"
                    ? { mode: "browser", email: effectiveEmail, browserRoot: value.browserRoot }
                    : { mode: "file", email: effectiveEmail, storageStatePath: value.storageStatePath })
                }, function (result) {
                  if (!result.ok) return "导入失败"
                  return importMode[0] === "login" ? "登录完成，账户已写入并重启内核" : "账户导入完成，内核已重启"
                })
              }
            }, busy === "import"
              ? (importMode[0] === "login" ? "等待登录…" : "导入中…")
              : (importMode[0] === "login" ? "打开登录窗口" : "导入账户"))
          ),
