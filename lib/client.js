...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AIStudioPage(props) {
      ...
      // 隔离登录不需要邮箱（由登录窗口读出），另外两种方式必须先填邮箱。
      var canImport = importMode[0] === "login"
        ? true
        : !!(value.accountEmail && (importMode[0] === "file" ? value.storageStatePath : value.browserRoot))
        ...
        h(
          ...
          h(
            ...
            h(Button, {
              ...
              onClick: function () {
                run("import", function () {
                  if (importMode[0] === "login") return call("/account/import", "POST", { mode: "login" })
                  return call("/account/import", "POST", importMode[0] === "browser"
                    ? { mode: "browser", email: value.accountEmail, browserRoot: value.browserRoot }
