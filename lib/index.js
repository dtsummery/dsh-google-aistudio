...
export function apply(ctx, entryConfig = {}) {
   ...
   * 把已存在的同名账户目录挪到一边。
   *
   * 内核的 setup 用「先写临时目录、再原子 rename」的方式建账户目录；Windows 上
   * rename 到一个已存在的目录会直接 Access denied，于是「登录态失效后重新登录」这条
   * 最主要的用例必然失败。这里先把旧目录改名留档（不删数据），让 rename 能落地。
   */
  const stashExistingAccount = (email) => {
    ...
    for (const name of readdirSync(join(dir, 'auth')).filter((n) => n.startsWith(`.replaced-${clean}-`))) {
      rmSync(join(dir, 'auth', name), { recursive: true, force: true })
    }
    const backup = join(dir, 'auth', `.replaced-${clean}-${Date.now()}`)
    renameSync(target, backup)
    log(`已把旧账户目录移到 ${backup}`)
  }
