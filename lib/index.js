...
export function apply(ctx, entryConfig = {}) {
  ...
  const maybeStartService = async () => {
    ...
    try {
      ...
      lastError = `启动数据面失败：${String(error?.message ?? error)}`
      warn(lastError)
    }
  }
  ...
  const browserRoots = () => {
    ...
    return [
      ...
      { id: 'profile', name: '插件状态目录（aistudio-profile 风格）', path: join(home, '.dsh', 'aistudio', 'browser-profile') },
      { id: 'roaming', name: 'Roaming 目录', path: roaming }
  ...
  ctx.inject(['webServer'], (sub) => {
    ...
    reg({
      ...
      handler: guard(async (_req, res) => {
        const instance = await ensureKernel()
        sendJson(res, 200, await instance.controlStart())
      })
    })
