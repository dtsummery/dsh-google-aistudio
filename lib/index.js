...
export function apply(ctx, entryConfig = {}) {
  ...
  const ensureKernel = () => {
    ...
    starting = (async () => {
      ...
      writeKernelEnv(port)
      // 内核启动前先把 auth/ 里的历史留档挪走，否则会被当成失效账户加载。
      relocateStashedAccounts()
   ...
   * 被替换下来的旧账户目录的留档位置。
   *
   * 必须放在 `auth/` 之外：内核按目录扫描 `auth/` 来发现账户，留在里面会被当成一个
   * 真实账户加载，于是状态里凭空多出一个「登录态失效」的账号，还可能被调度到。
   */
  const stashDir = join(dir, '.replaced')

  const stashExistingAccount = (email) => {
    const clean = String(email ?? '').trim()
    if (!clean) return
    const target = join(dir, 'auth', clean)
    if (!existsSync(target)) return
    mkdirSync(stashDir, { recursive: true })
    // 同一个邮箱只保留最近一份留档，避免反复登录把目录堆满。
    for (const name of readdirSync(stashDir).filter((n) => n.startsWith(`${clean}-`))) {
      rmSync(join(stashDir, name), { recursive: true, force: true })
    }
    const backup = join(stashDir, `${clean}-${Date.now()}`)
    renameSync(target, backup)
    log(`已把旧账户目录留档到 ${backup}`)
  }

  /** 把早期版本遗留在 `auth/` 里的 `.replaced-*` 留档挪到 `stashDir`。 */
  const relocateStashedAccounts = () => {
    const authDir = join(dir, 'auth')
    if (!existsSync(authDir)) return
    for (const name of readdirSync(authDir)) {
      if (!name.startsWith('.replaced-')) continue
      mkdirSync(stashDir, { recursive: true })
      const target = join(stashDir, name.replace(/^\.replaced-/, ''))
      rmSync(target, { recursive: true, force: true })
      try {
        renameSync(join(authDir, name), target)
        log(`已把留档目录移出 auth：${name}`)
      } catch (error) {
        warn(`移出留档目录失败（可能仍被占用）：${String(error?.message ?? error)}`)
      }
    }
  }
  ...
  ctx.inject(['webServer'], (sub) => {
    ...
    reg({
      ...
      handler: guard(async (req, res) => {
        ...
        // 内核运行时会占用账户目录里的文件，先停掉它，导入完成后再拉起来。
        if (kernel !== null) {
          await kernel.shutdown().catch(() => {})
          kernel = null
        }
        stashExistingAccount(email)
