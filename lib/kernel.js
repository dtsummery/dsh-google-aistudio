...
export class Kernel {
  ...
  /**
   * 结束实例：托管实例直接结束进程树；复用来的残留实例按监听端口反查 PID 再结束。
   *
   * 先请内核自己收尾（它会关闭 WAA Worker 与随之拉起的 Camoufox），再整树结束进程 ——
   * 只 kill 内核主进程会把 Camoufox 留在后台变成孤儿进程。
   *
  ...
  async shutdown() {
    if (this.child) {
      await this.controlStop().catch(() => {})
      const pid = this.child.pid
      if (process.platform === 'win32' && pid) {
        await new Promise((resolve) => {
          execFile('taskkill', ['/PID', String(pid), '/T', '/F'], { windowsHide: true, timeout: 20000 }, () => resolve(undefined))
        })
      } else {
        try {
          this.child.kill()
